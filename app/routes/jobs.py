...
def create_job():
    ...
    # SIEMPRE generamos un job_id lógico para el frontend
...
def get_job(job_id: str):
    if AudioJob is None:
        return jsonify({"id": job_id, "job_id": job_id, "status": "done"}), 200
...
def history_api():
    ...
    if AudioJob is not None:
        try:
            ...
            rows = q.order_by(getattr(AudioJob, "created_at").desc()).limit(limit).all()
            for r in rows:
                items.append(
                    {
